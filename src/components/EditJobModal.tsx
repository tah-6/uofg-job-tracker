...
export default function EditJobModal({ open, row, onClose, onUpdate }: Props) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-lg rounded-2xl bg-white p-5 shadow-xl">
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-lg font-semibold">Edit Job (modal is mounted)</h2>
          <button onClick={onClose} className="rounded px-2 py-1 text-sm text-gray-600 hover:bg-gray-100">Close</button>
        </div>
        <form onSubmit={submit} className="grid grid-cols-1 gap-3">
          <input className="rounded border px-3 py-2" placeholder="Company" value={form.company}
                 onChange={(e)=>setForm(f=>({...f,company:e.target.value}))}/>
          <input className="rounded border px-3 py-2" placeholder="Position" value={form.position}
                 onChange={(e)=>setForm(f=>({...f,position:e.target.value}))}/>
          <div className="grid grid-cols-2 gap-3">
            <label className="text-sm text-gray-600">
              <span className="mb-1 block">Date Applied</span>
              <input type="date" className="w-full rounded border px-3 py-2" value={form.dateApplied}
                     onChange={(e)=>setForm(f=>({...f,dateApplied:e.target.value}))}/>
            </label>
            <label className="text-sm text-gray-600">
              <span className="mb-1 block">Deadline</span>
              <input type="date" className="w-full rounded border px-3 py-2" value={form.deadline}
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <input className="rounded border px-3 py-2" placeholder="Portal URL" value={form.portal}
                   onChange={(e)=>setForm(f=>({...f,portal:e.target.value}))}/>
          </div>
          <textarea className="min-h-[80px] rounded border px-3 py-2" placeholder="Details / notes" value={form.details}
