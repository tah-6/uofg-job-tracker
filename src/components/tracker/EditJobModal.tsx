...
export default function EditJobModal({
  ...
  const [form, setForm] = useState({
    ...
    status: "saved" as JobStatus, details: "", portal: "", resumeVersion: "",
  });
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-lg rounded-2xl bg-white p-5 shadow-xl dark:bg-slate-800 dark:text-slate-100">
        <div className="mb-4 flex items-center justify-between">
          ...
          <button
            onClick={onClose}
            className="rounded px-2 py-1 text-sm text-gray-600 hover:bg-gray-100 dark:hover:bg-slate-700"
          >
            Close
          </button>
        </div>
        ...
        <form onSubmit={submit} className="grid grid-cols-1 gap-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <label className="text-sm text-gray-600 dark:text-slate-300">
              ...
              >
                {(
                  ["saved", "submitted", "in_progress", "interview", "offer", "rejected"] as JobStatus[]
                ).map((s) => (
          ...
          <label className="text-sm text-gray-600 dark:text-slate-300">
            ...
            <input
              ...
              onChange={(e) =>
                setForm((f) => ({ ...f, resumeVersion: e.target.value }))
              }
            />
          </label>
          <textarea
          ...
          <div className="mt-1 flex justify-end gap-2">
            <button
              type="button"
              onClick={onClose}
              className="rounded border px-3 py-2 hover:bg-gray-50 dark:hover:bg-slate-700"
            >
              Cancel
            </button>
            <button
